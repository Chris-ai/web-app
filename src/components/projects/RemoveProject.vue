<template>
  <div class="add-new-project">
    <!-- <span>:D</span> -->
    <h3>Czy na pewno chcesz usunąć {{ removeProject.body }}?</h3>
    <form @submit="remove">
      <div
        class="wrap-label"
...
export default {
  ...
  methods: {
    back() {
      this.$emit("removeProject", false);
    },
    remove() {
      this.$emit("removeProject", false);
      console.log(this.patient);

      ProjectService.removeProject(this.removeProject.id)
        .then((res) => {
          this.$emit("message", "Pomyślnie usunięto projekt");
