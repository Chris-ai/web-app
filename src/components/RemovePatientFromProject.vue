<template>
  <div class="add-new-patient">
    <!-- <span>:D</span> -->
    <h3>
      Czy na pewno chcesz usunąć {{ removePatient.firstName }}
      {{ removePatient.lastName }} z tego projektu?
    </h3>
...
export default {
  ...
  mounted() {
    ProjectService.getProjectById(this.projectId)
      .then((res) => {
        this.project = JSON.parse(JSON.stringify(res.data));
        console.log("data", JSON.parse(JSON.stringify(res.data)));
      })
      .catch((e) => console.log(e));
  },
  methods: {
    async removeItemFromList(list, item) {
      return list.filter((el) => el.id !== item);
    },
    back() {
      this.$emit("removePatient", false);
    },
    remove() {
      console.log("remove patient: ", this.removePatient);
      this.removeItemFromList(this.project.patients, this.removePatient.id)
        .then((res) => {
          // console.log(res);
          this.project.patients = res;
          ProjectService.updateProject(this.project.id, this.project).then(
            (res) => {
              console.log(res.data);
              this.$emit("message", "Pomyślnie usunięto pacjenta");
